...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x - 6.f * dpi, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x - 6.f * dpi, ImLerp(bb.Min.y - 6 * dpi, bb.Max.y, grab_v_norm) + grab_h_pixels);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg * dpi, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y)); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::widget::bg_inactive), c::widget::rounding);

        PushFont(font::icomoon);
        GetWindowDrawList()->AddText(frame_bb.Max - ImVec2((size_arg.y * dpi + CalcTextSize(label).x) / 2, (size_arg.y * dpi + CalcTextSize(label).x) / 2), GetColorU32(c::accent), label);
        PopFont();
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x - size_arg.y * dpi, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2((size_arg.y * dpi - CalcTextSize(hint).y), (size_arg.y * dpi - CalcTextSize(hint).y)) / 2;
